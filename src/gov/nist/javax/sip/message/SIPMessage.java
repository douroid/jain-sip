 ...
 *
 */
public abstract class SIPMessage extends MessageObjectImpl implements SIPMessageInt, javax.sip.message.Message,
        MessageExt {
    ...
    public final String getDialogId(boolean isServer, Buffer toTag) {
        ...
        StringBuilder retval = new StringBuilder(cid.getCallId().length() + from.getTag().length() + 13);
    ...
    public SIPMessage(SipMessage impl) {
...
//        }
        ...
        if (topVia != null
            ...
            if (this.getCSeq().getMethod().equals(Request.CANCEL)) {
                retval.append(topVia.getBranch());
                retval.append(":");
                retval.append(this.getCSeq().getMethod());
            } else {
                retval.append(viaBranch);
            }
        } else {

...
public class SIPRequestImpl extends SIPMessageImpl implements SIPRequest, javax.sip.message.Request, RequestExt {
    ...
    static {
        ...
        headersToIncludeInResponse.add(FromHeader.NAME);
        headersToIncludeInResponse.add(ToHeader.NAME);
        headersToIncludeInResponse.add(ViaHeader.NAME);
        headersToIncludeInResponse.add(RecordRouteHeader.NAME);
        headersToIncludeInResponse.add(CallIdHeader.NAME);
        headersToIncludeInResponse.add(CSeqHeader.NAME);
        headersToIncludeInResponse.add(TimeStampHeader.NAME);
    }
    ...
    public String debugDump() {
        ...
        sprint(SIPRequestImpl.class.getName());
        sprint("{");
        if (requestLine != null) {
    ...
    public void checkHeaders() throws ParseException {
        ...
        if (request.getCallIDHeader() == null || request.getCallIDHeader().getCallId() == null
                || request.getCallIDHeader().getCallId().equals("")) {
    ...
    public void setMethod(String method) {
        ...
        if (request.getCSeqHeader() != null) {
            /*try {
             //TODO request.getCSeqHeader().setMethod(meth);
     ...
     */
    public Object clone() {
        SIPRequestImpl retval = (SIPRequestImpl) super.clone();
    ...
    public boolean equals(Object other) {
        ...
        SIPRequestImpl that = (SIPRequestImpl) other;
    ...
    public boolean match(Object matchObj) {
        ...
        SIPRequestImpl that = (SIPRequestImpl) matchObj;
    ...
    public byte[] encodeAsBytes(String transport) {
        return msgImpl.toBuffer().getArray();
    }
    ...
    public SIPResponse createResponse(int statusCode, String reasonPhrase) {
        ...
        newResponse = new SIPResponseImpl(request.createResponse(statusCode));
...
//        }
        /*TODO ServerHeader server = MessageFactoryImpl.getDefaultServerHeader();
        if (server != null) {
     ...
     */
    public SIPRequestImpl createCancelRequest() throws SipException {
         ...
         */
        SIPRequestImpl cancel = new SIPRequestImpl();
        cancel.setRequestLine((RequestLine) this.requestLine.clone());
        cancel.setMethod(Request.CANCEL);
        cancel.setHeader((Header) request.getCallIDHeader().clone());
        cancel.setHeader((Header) request.getToHeader().clone());
        cancel.setHeader((Header) request.getCSeqHeader().clone());
        try {
            cancel.getCSeq().setMethod(Request.CANCEL);
        } catch (ParseException e) {
            e.printStackTrace(); // should not happen
        }
        cancel.setHeader((Header) request.getFromHeader().clone());

        cancel.addFirst((Header) request.getViaHeader().clone());
        cancel.setHeader((Header) request.getMaxForwards().clone());
     ...
     */
    public SIPRequestImpl createAckRequest(To responseToHeader) {
...
//        SIPHeader nextHeader;
        ...
        SIPRequestImpl newRequest = (SIPRequestImpl) this.clone();
...
//        newRequest.setRequestLine((RequestLine) this.requestLine.clone());
     ...
     * @author jvb
     */
    public final SIPRequestImpl createErrorAck(To responseToHeader) throws SipException,
            ParseException {
         ...
         * be equal to "ACK".
         */
        SIPRequestImpl newRequest = new SIPRequestImpl();
        newRequest.setRequestLine((RequestLine) this.requestLine.clone());
        newRequest.setMethod(Request.ACK);
        newRequest.setHeader((Header) request.getCallIDHeader().clone());
        newRequest.setHeader((Header) request.getMaxForwards().clone()); // ISSUE
        // 130
        // fix
        newRequest.setHeader((Header) request.getFromHeader().clone());
        newRequest.setHeader((Header) responseToHeader.clone());
        newRequest.addFirst((Header) request.getViaHeader().clone());
        newRequest.setHeader((Header) request.getCSeqHeader().clone());
        newRequest.getCSeq().setMethod(Request.ACK);
    ...
    public String getViaHost() {
        ViaHeader via = this.getViaHeaders().getFirst();
        return via.getHost();

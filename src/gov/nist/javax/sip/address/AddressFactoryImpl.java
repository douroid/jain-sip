...
public class AddressFactoryImpl implements AddressFactoryEx {
    ...
    /**
     * Creates an Address with the new address string value. The address string
     ...
     * determined if <code>((SipURI)Address.getURI).getUser() == *;</code>.
     *
    ...
    public javax.sip.address.Address createAddress(String address)
        ...
        try {
            final Object[] result = AddressParametersHeader.frame(Buffers.wrap(address));
            return (Address) result[0];
        } catch (SipParseException | IndexOutOfBoundsException E) {
    ...
    /**
     * Creates a TelURL based on given URI string. The '+' should not be
     * included in the phoneNumber string argument. If scheme is not present, it
     * will be added.
     *
